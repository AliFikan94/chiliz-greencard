...
export function ConnectWalletButton({ sessionKey }: { sessionKey: string | null }) {
  ...
  if (isConnected && address) {
    return (
      <div className="flex items-center gap-2">
        ...
        {bindError && <span className="text-xs text-red-600">{bindError}</span>}
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className="rounded-full bg-black px-4 py-2 text-sm font-semibold text-white transition hover:scale-[1.02] disabled:opacity-50"
    >
